  ...
  Doctor ID • {formData.doctorID}
          ...
          <div className="flex-1 rounded-[2.5rem] bg-white border-2 border-slate-100 p-10 flex items-center justify-around shadow-sm">
            ...
            <div className="text-center">
              ...
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mt-1">Rating</p>
            </div>
            <div className="h-12 w-px bg-slate-100" />
            <div className="text-center">
              <p className="text-4xl font-black text-black">150+</p>
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mt-1">Patients</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-6 h-[70%]">
          <section className="flex-[2] rounded-[2.5rem] bg-white border-2 border-slate-100 p-12 shadow-sm flex flex-col">
            <div className="flex justify-between items-center mb-12">
              ...
              <button
                ...
                className={`px-8 py-3 rounded-2xl font-black uppercase text-xs tracking-widest transition-all ${
                  isEditing ? "bg-green-600 text-white shadow-lg shadow-green-200" : "bg-[#006666] text-white hover:bg-slate-800"
                }`}
              >
            ...
            <div className="grid grid-cols-2 gap-x-16 gap-y-10">
              <DoctorDetail label="Medical License #" name="licenseNumber" value={formData.licenseNumber} isEditing={isEditing} onChange={handleChange} />
              <DoctorDetail label="Primary Specialization" name="specialization" value={formData.specialization} isEditing={isEditing} onChange={handleChange} />
              <DoctorDetail label="Clinical Affiliation" name="hospital" value={formData.hospital} isEditing={isEditing} onChange={handleChange} />
              <DoctorDetail label="Contact Extension" name="phone" value={formData.phone} isEditing={isEditing} onChange={handleChange} />
              <DoctorDetail label="Professional Email" name="email" value={formData.email} isEditing={isEditing} onChange={handleChange} />
              <DoctorDetail label="Consultation Fee ($)" name="fee" value={formData.fee} isEditing={isEditing} onChange={handleChange} />
            </div>
          </section>
          ...
          <div className="flex-1 flex flex-col gap-6">
            <section className="flex-1 rounded-[2.5rem] bg-[#006666] p-10 text-white shadow-2xl relative overflow-hidden flex flex-col justify-center">
              <div className="absolute -right-6 -bottom-6 opacity-10 text-9xl font-black rotate-12">🩺</div>
              ...
              <div className="space-y-6 relative z-10">
                <div className="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm">
                  <p className="text-[10px] font-black uppercase opacity-60 mb-1">Current Cycle</p>
                  <p className="text-xl font-bold">Mon — Fri • 09:00 - 17:00</p>
                </div>
                <div className="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm">
                  <p className="text-[10px] font-black uppercase opacity-60 mb-1">On-Call Status</p>
                  <p className="text-xl font-bold text-teal-300">Available for Emergency</p>
                </div>
              </div>
            </section>
          </div>
...
function DoctorDetail({ label, name, value, isEditing, onChange }) {
  return (
    <div className="flex flex-col gap-2">
      <p className="text-[15px] font-black text-black font-semibold uppercase tracking-[0.2em]">{label}</p>
      {isEditing ? (
        <input
          name={name}
          value={value || ""}
