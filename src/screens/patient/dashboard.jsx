...
const futureAppointments = appointments.filter(a => {
    ...
    <div className="h-[calc(100vh-80px)] w-full overflow-hidden px-20 bg-slate-50">
      <div className=" h-full grid grid-cols-12 grid-rows-6 gap-8">
        ...
        <section className="col-span-8 row-span-4 rounded-[3rem] bg-white border-2 border-slate-100 p-12 shadow-sm flex flex-col justify-center relative overflow-hidden">
          <div className="absolute -top-10 -right-10 opacity-[0.03] text-[20rem] font-black pointer-events-none text-[#009999]">🩺</div>
          <div className="relative z-10">
            ...
            <p className="text-xl text-slate-500 max-w-lg mb-10 leading-relaxed">
              Your clinical dashboard is ready. Analyze symptoms with AI or browse the medical library.
            </p>
            <div className="flex gap-6">
              <Link to="/chat" className="rounded-2xl bg-[#009999] px-10 py-5 text-base font-bold text-white shadow-2xl shadow-[#009999]/30 hover:scale-105 active:scale-95 transition-all">
                Start AI Triage
              </Link>
              <Link to="/library" className="rounded-2xl border-2 border-slate-200 bg-white px-10 py-5 text-base font-bold text-slate-600 hover:border-[#009999] hover:text-[#009999] transition-all">
                Medical Library
              </Link>
            </div>
          </div>
  ...
  <div className="relative z-10">
    ...
    <div className="grid grid-cols-2 gap-y-6 text-sm font-semibold">
      <div>
        <p className="text-slate-300 uppercase text-[10px] font-black tracking-wider">Age</p>
        <p className="text-black">{profile?.age || "—"}</p>
      </div>

      <div>
        <p className="text-slate-300 uppercase text-[10px] font-black tracking-wider">Blood Group</p>
      ...
      <div>
        <p className="text-slate-300 uppercase text-[10px] font-black tracking-wider">Phone</p>
        <p className="text-black">{profile?.phone || "—"}</p>
      </div>

      <div>
        <p className="text-slate-300 uppercase text-[10px] font-black tracking-wider">Location</p>
